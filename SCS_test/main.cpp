...
Q* create_queue() {
	...
	sq_ptr->size = 0;
	sq_ptr->max_size = PAGE_SIZE;

	if (sq_ptr->start_id + sq_ptr->max_size > MEMORY_SIZE) {
		on_out_of_memory();
		queue_counter--;
		return nullptr;
	}

	//std::cout << "Sub queue created at: " << sq_ptr << "\nsize: " << sq_ptr->size << std::endl;
...
void enqueue_byte(Q* q, unsigned char b) {
	...
	if (sq_ptr->size >= sq_ptr->max_size) {
		on_out_of_memory();
	}
	...
	std::uint16_t relative_offset = sq_ptr->size % sq_ptr->max_size;
...
unsigned char dequeue_byte(Q* q) {
	...
	size_t partition_start = DATA_POOL_START + ((sq_ptr - reinterpret_cast<SubQueue*>(data)) * PAGE_SIZE);

	sq_ptr->start_id = partition_start + ((sq_ptr->start_id - partition_start + 1) % sq_ptr->max_size);
	sq_ptr->size--;
